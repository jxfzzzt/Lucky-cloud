...
public class TCPSocketTemplate extends AbstractRemoteServer implements ApplicationRunner {
    ...
    private synchronized void startAsync() {
        ...
        try {
            bindPorts();
            ready.set(true);
            log.info("TCP 服务器启动完毕 端口: {}", channelFutures.keySet());
        } catch (Exception ex) {
    ...
    private void bindPorts() {
        ...
        try {
            nacosRegistrationService.registerTcpPorts(List.copyOf(channelFutures.keySet()));
        } catch (Exception e) {
            log.error("TCP 端口注册到 Nacos 失败", e);
        }
