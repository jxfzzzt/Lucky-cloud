 ...
 * - 支持将 protobuf Message pack 进 Any（Any.pack）
 * - 支持稳健的 Any 解包（优先 protobuf 二进制解析，失败回退为 JSON 文本解析，再回退为原始 bytes）
 * <p>
 * 设计原则：
 * - 兼容性优先：能兼容后端把 JSON 文本直接放进 Any.value（例如 value 为 "\"registrar\""）
...
public final class ProtoJsonUtils {
    ...
    public static Any packAny(Object data) {
        ...
        if (isPrimitiveOrString(data) || data instanceof Map || data instanceof List) {
            ...
            // 若是 List 或 primitive，统一包装成 Struct 的 fields: { "value": v }，保持兼容性
        ...
        try {
            ...
            // 万一 Jackson 无法转换（通常不会），兜底：把 data 转成 JSON 字符串然后用 JsonFormat 解析到 Struct
            try {
                ...
                // 最后兜底：pack 空 Struct 并记录日志（调用方可检测异常）
    ...
    /**
     * 把 com.google.protobuf.Any 解包成 Java 对象（Map/List/primitive 或 byte[]）
     * <p>
     * 解析策略（按优先级）：
    ...
    public static Object unpackAny(Any any) {
        ...
        // 回退 1：尝试把 raw 当作 UTF-8 JSON 文本解析（处理服务端把 JSON bytes 放进 Any.value 的情况）
        try {
        ...
        // 回退 2：使用 JsonFormat 打印 Any 为 JSON 表示，再用 Jackson 解析
        try {
    ...
    public static Value javaToValue(Object o) {
        ...
        if (o instanceof Number) {
            // 统一转为 double（protobuf Value.number_value 为 double）
