...
public class AuthHandler extends ChannelInboundHandlerAdapter {
    ...
    @Autowired
    private ClientInboundMessageHandler clientInboundMessageHandler;
    ...
    private void ensurePostAuthPipeline(ChannelHandlerContext ctx) {
        ...
        if (pipeline.get(HeartBeatHandler.class) == null) {
            pipeline.addLast("heartBeat", heartBeatHandler);
        }
        if (pipeline.get(LoginHandler.class) == null) {
            pipeline.addLast("login", loginHandler);
        }
        if (pipeline.get(ClientInboundMessageHandler.class) == null) {
            pipeline.addLast("clientInbound", clientInboundMessageHandler);
        }
